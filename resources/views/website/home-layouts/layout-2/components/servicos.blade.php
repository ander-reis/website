<div class="row">
    <div class="col-md-12">
        <div class="row">
            <div class="col-lg-3 col-md-6 col-sm-6 pt-1">
                <div class="p-1 servicos1">
                    <a href="{{route('paginas-principais', ['url-pagina' => 'assistencia-juridica'])}}">
                        <div class="row">

                            <div class="col-3 text-center">
                                <i class="fas fa-balance-scale fa-3x text-dark"></i>
                            </div>

                            <div class="col-9 pl-4 my-auto">
                                <span class="text-dark servicos_size">ATENDIMENTO<br />JURÍDICO</span>
                            </div>
                        </div>
                    </a>
                </div>
            </div>
            <div class="col-lg-3 col-md-6 col-sm-6 pt-1">
                <div class="p-1 servicos1">
                    <a href="{{route('paginas-principais', ['url-pagina' => 'assistencia-previdenciaria'])}}">
                        <div class="row">
                            <div class="col-3 text-center">
                                <i class="fas fa-tasks fa-3x text-dark"></i>
                            </div>
                            <div class="col-9 pl-4 my-auto">
                                <span class="text-dark servicos_size"> ATENDIMENTO<br />PREVIDENCIÁRIO</span>
                            </div>
                        </div>
                    </a>
                </div>
            </div>
            <div class="col-lg-3 col-md-6 col-sm-6 pt-1">
                <div class="p-1 servicos1">
                    <a href="{{route('paginas-principais', ['url-pagina' => 'atendimento-medico'])}}">
                        <div class="row">
                            <div class="col-3 text-center">
                                <i class="fas fa-user-md fa-3x text-dark"></i>
                            </div>
                            <div class="col-9 pl-4 my-auto">
                                <span class="text-dark servicos_size"> ATENDIMENTO <br />
                                    MÉDICO</span>
                            </div>
                        </div>
                    </a>
                </div>
            </div>
            <div class="col-lg-3 col-md-6 col-sm-6 pt-1">
                <div class="p-1 servicos1">
                    <a href="{{route('paginas-principais', ['url-pagina' => 'colonia'])}}">
                        <div class="row">
                            <div class="col-3 text-center">
                                <i class="fas fa-umbrella-beach fa-3x text-dark"></i>
                            </div>
                            <div class="col-9 pl-4 my-auto">
                                <span class="text-dark servicos_size"> COLÔNIA <br />
                                    DE FÉRIAS</span>
                            </div>
                        </div>
                    </a>
                </div>
            </div>
        </div>
    </div>

    <div class="col-md-12">
        <div class="row">
            <div class="col-lg-3 col-md-6 col-sm-6 pt-1">
                <div class="p-1 servicos1">
                    <a href="{{route('fono.index')}}">
                        <div class="row">
                            <div class="col-3 text-center">
                                <i class="fab fa-creative-commons-sampling fa-3x text-dark"></i>
                            </div>
                            <div class="col-9 pl-4 my-auto">
                                <span class="text-dark servicos_size">FONOAUDIOLOGIA</span>
                            </div>
                        </div>
                    </a>
                </div>
            </div>
            <div class="col-lg-3 col-md-6 col-sm-6 pt-1">
                <div class="p-1 servicos1">
                    <a href="http://planoprofessor.com.br/" target="_blank">
                        <div class="row">
                            <div class="col-3 text-center">
                                <i class="fas fa-notes-medical fa-3x text-dark"></i>
                            </div>
                            <div class="col-9 pl-4 my-auto">
                                <span class="text-dark servicos_size">PLANO DE SAÚDE</span>
                            </div>
                        </div>
                    </a>
                </div>
            </div>
            <div class="col-lg-3 col-md-6 col-sm-6 pt-1">
                <div class="p-1 servicos1">
                    <a href="http://www1.sinprosp.org.br/convenios.asp" target="_blank">
                        <div class="row">
                            <div class="col-3 text-center">
                                <i class="fas fa-handshake fa-3x text-dark"></i>
                            </div>
                            <div class="col-9 pl-4 my-auto">
                                <span class="text-dark servicos_size"> CONVÊNIOS <br />
                                    E PARCERIAS</span>
                            </div>
                        </div>
                    </a>
                </div>
            </div>
            <div class="col-lg-3 col-md-6 col-sm-6 pt-1">
                <div class="p-1 servicos1">
                    <a href="http://www1.sinprosp.org.br" target="_blank">
                        <div class="row">
                            <div class="col-3 text-center">
                                <i class="fas fa-mobile-alt fa-3x text-dark"></i>
                            </div>
                            <div class="col-9 pl-4 my-auto">
                                <span class="text-dark servicos_size"> APLICATIVO <br />
                                    DO SINPROSP</span>
                            </div>
                        </div>
                    </a>
                </div>
            </div>
        </div>
    </div>

    <div class="col-md-12">
        <div class="row">
            <div class="col-lg-3 col-md-6 col-sm-6 pt-1">
                <div class="p-1 servicos1">
                    <a href="http://www1.sinprosp.org.br/atendimento.asp" target="_blank">
                        <div class="row">
                            <div class="col-3 text-center">
                                <i class="fas fa-comments fa-3x text-dark"></i>
                            </div>
                            <div class="col-9 pl-4 my-auto">
                                <span class="text-dark servicos_size"> ATENDIMENTO<br />
                                    ELETRÔNICO</span>
                            </div>
                        </div>
                    </a>
                </div>
            </div>
            <div class="col-lg-3 col-md-6 col-sm-6 pt-1">
                <div class="p-1 servicos1">
                    <a href="http://www1.sinprosp.org.br/boletim_eletronico.asp" target="_blank">
                        <div class="row">
                            <div class="col-3 text-center">
                                <i class="fas fa-envelope-open-text fa-3x text-dark"></i>
                            </div>
                            <div class="col-9 pl-4 my-auto">
                                <span class="text-dark servicos_size"> RECEBA O BOLETIM<br />
                                    DO SINPRO</span>
                            </div>
                        </div>
                    </a>
                </div>
            </div>
            <div class="col-lg-3 col-md-6 col-sm-6 pt-1">
                <div class="p-1 servicos1">
                    <a href="http://www1.sinprosp.org.br" target="_blank">
                        <div class="row">
                            <div class="col-3 text-center">
                                <i class="fas fa-folder-plus fa-3x text-dark"></i>
                            </div>
                            <div class="col-9 pl-4 my-auto">
                                <span class="text-dark servicos_size"> ACERVO</span>
                            </div>
                        </div>
                    </a>
                </div>
            </div>
            <div class="col-lg-3 col-md-6 col-sm-6 pt-1">
                <div class="p-1 servicos1">
                    <a href="{{route('paginas-principais', ['url-pagina' => 'homologacao'])}}">
                        <div class="row">
                            <div class="col-3 text-center">
                                <i class="fas fa-copy fa-3x text-dark"></i>
                            </div>
                            <div class="col-9 pl-4 my-auto">
                                <span class="text-dark servicos_size"> HOMOLOGAÇÃO</span>
                            </div>
                        </div>
                    </a>
                </div>
            </div>
        </div>
    </div>
    <div>
